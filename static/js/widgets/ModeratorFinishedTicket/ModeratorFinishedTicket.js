...
import ModeratorFinishedTicketModel from "./ModeratorFinishedTicketModel";

import { Input, message, Space, Button, PageHeader } from "antd";

const resourceServer = config.resourceServer;

const ModeratorFinishedTicket = () => {
  ...
  let [moderatorFinishedTicketData, setModeratorFinishedTicketData] = useState(
    []
  );
  ...
  function requestCallback(res) {
    if (res.status == 200) {
      ...
      setModeratorFinishedTicketData(res.data.data.items);
  ...
  if (moderatorId) {
    pageContent = (
      <>
        <PageHeader
          ...
          title={
            t("Moderator") + `  (${moderatorId})  ` + t("Submitted Tickets")
          }
        />
        <Space>
        ...
        <ModeratorFinishedTicketModel
          dataList={moderatorFinishedTicketData}
          total={total}
    ...
    pageContent = "Error : No Moderator";
  }
  return pageContent;
};
...
function getData(accessToken, moderatorId, timeRange, page, ticketId, pageSize) {
  ...
  let url = resourceServer.GetModeratorSubmittedTicketData;
  return axios({
    method: "get",
    url,
    params,
...
export default ModeratorFinishedTicket;
