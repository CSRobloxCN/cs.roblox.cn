...
const TicketInfo = ({ ticket }) => {
  ...
  let [userAccuseCount, setUserAccuseCount] = useState(0);
      ...
      loading(
        getUserInGameAbuseReportCount(
          ...
          if (res && res.status == 200 && !res.data.error) {
            setUserAccuseCount(res.data.data);
          }
        })
      );
    }
...
function renderUserDesc(userInfo, title, t, userAccuseCount, isAccused) {
    ...
    <Descriptions bordered size="small" title={title}>
      ...
      {isAccused && (
        <Descriptions.Item label={t("Accuse Count(past week)")}>
          <span style={{ color: userAccuseCount >= 3 ? "#f00" : "inherit" }}>
            {userAccuseCount}
          </span>
        </Descriptions.Item>
      )}
...
function getUserInGameAbuseReportCount(userId, startUtc, endUtc, accessToken) {
  return axios({
    method: "get",
    url:
      config.resourceServer.GetUserInGameAbuseReport +
      "/accused-user-id/" +
      userId +
      "/count?startUtc=" +
      startUtc +
